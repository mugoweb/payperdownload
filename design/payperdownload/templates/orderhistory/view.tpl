{* Copied from shop/customerorderview.tpl *}
...
{section var="Product" loop=$product_list sequence=array(bglight,bgdark)}
    ...
    <tr class="{$Product.sequence}">
        <td>{content_view_gui view=text_linked content_object=$Product.product}
        {def $download_classes = ezini('PayPerDownloadSettings', 'ContentClasses', 'payperdownload.ini')}
        {if $download_classes|contains($Product.product.class_identifier)}
            {def $download_attributes = ezini('PayPerDownloadSettings', 'Attributes', 'payperdownload.ini')}
            {def $download_attribute = $download_attributes[$Product.product.class_identifier]}
            <br />
            - Download: <a href={concat('content/download/',$Product.product.data_map.$download_attribute.content.id,'/',$Product.product.data_map.$download_attribute.content.data_map.file.id)|ezurl()}>{$Product.product.data_map.$download_attribute.content.name}</a>
            {undef $download_attributes}
            {undef $download_attribute}
        {/if}
        {undef $download_classes}
        </td>
